 ...
 * @return {Array}
 */
Couchbase.prototype.getKeysFromWhere = function (model, where) {
...
Couchbase.prototype.invokeForKeys = function (method, keys, options) {
  ...
  return Promise.map(keys, function (key) {
    return connection.call(method, key, options);
  }).map(this.unpackRes);
};
...
Couchbase.prototype.all = function all(model, query, options, callback) {
  ...
  if (keys) {
    var promise = this.invokeForKeys('getAsync', keys, options);
  } else {
    // TODO: Do query
    return Promise.reject(new Error('...'));
  }
...
Couchbase.prototype.destroyAll = function destroyAll(model, where, options, callback) {
  ...
  if (keys) {
    var promise = this.invokeForKeys('removeAsync', keys, options);
  } else {
    // TODO: Do query
    return Promise.reject(new Error('...'));
  }
...
Couchbase.prototype.ping = function (callback) {
  var promise;
  if (this._bucket == null) {
    promise = Promise.reject(new Error('already disconnected'));
  }else {
    var connection = this.connect();
    promise = connection.call('getAsync', '1').then(function (res) {
      return true;
    }).catch(function (err) {
