...
class Couchbase extends NoSQL {
  ...
  all(modelName, query, options, callback) {
    ...
    const ids = super.getIdsFromWhere(modelName, query.where);
    // Result must be an array.
    let promise;
    if (ids) {
      promise = super.findByIds(modelName, ids, options);
    } else {
      promise = super.findByFilters(modelName, query, options);
    }
    return promise.asCallback(callback);
  }
};
...
class CouchbaseAccessor extends Accessor {
  ...
  /**
   * Ensure all fields ared declared in model
   * @param {Array} fields fields
   */
  checkFields(fields) {
    // TODO: get fields from the models setting
    return true;
  }
  findByFilters(filters, options) {
    ...
    if (filters.where) {
      ...
      if (this.checkFields(keys)) {
        ...
        // TODO, check fields in opeartor list
        // Or throw a synatx error
      }
    }
    if (filters.limit) {
