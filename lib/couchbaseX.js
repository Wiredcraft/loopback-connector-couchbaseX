...
class Couchbase extends NoSQL {
  ...
  _connect(settings, database) {
    ...
    // View.
    if (settings.stale == couchbase.ViewQuery.Update.BEFORE
      || settings.stale == couchbase.ViewQuery.Update.AFTER) {
...
class CouchbaseAccessor extends Accessor {
  ...
  postWithId(id, data, options) {
    ...
    return this.connection.call('insertAsync', id, record, options).then((res) => {
      return [id, res.cas];
    });
  }
   ...
   */
  findById(id, options) {
    return this.connection.call('getAsync', id, options).then(unpackRes);
  }
  ...
  findAll(options) {
    ...
    return this.connector.view('connector', 'byModelName', opts)
      .map(item => item.id)
      .then(ids => {
        if (!ids.length) {
